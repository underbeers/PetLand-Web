...
const AdCards: React.FC<iAdCardProps> = ({size}) => {
    ...
    return (
        <div className={cn(styles.card, styles[size])}>
            <img className={styles.photo} src={'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1143&q=80'}/>
            <div className={styles.ad__content}>
                <div className={styles.name__like}>
                    ...
                    {isLiked ?
                        ...
                        <Icons icon={'cards-heart-outline'} className={styles.heart} onClick={() => setIsLiked(!isLiked)} />}
                </div>
                {size === 'big' &&
                ...
                <div className={styles.description__info}>
                    {size === 'big' &&
                        ...
                        стерилизована, что делает ее идеальным питомцем для семьи. Кошка привыкла к лотку и когтеточке,
